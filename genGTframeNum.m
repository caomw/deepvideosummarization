...
gtLabels=cell(length(userSummDir)-2,2);
...
for v=3:length(userSummDir)
    ...
    gtLabels{v-2,2}=cell(length(userDir)-2,1);
    for u=3:length(userDir)
        ...
        gtLabels{v-2,2}{u-2}=sort(cell2mat(arrayfun(@(X) str2double(X.name(6:end-4)),gtDir(3:end),'uniformoutput',false)),'ascend');
    end
end
