...
class EuclideanLossLayer : public LossLayer<Dtype> {
 ...
 protected:
  ...
  Blob<Dtype> diff_;
  Blob<Dtype> L2dist_;
  Blob<Dtype> exp1overL2dist_;
  Blob<Dtype> all1s_;
  Blob<Dtype> sumexp1overL2dist_;
  Blob<Dtype> gtdiff_;
  Blob<Dtype> gtL2dist_;
};

template <typename Dtype>
class OurLossLayer : public LossLayer<Dtype> {
 public:
  explicit OurLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "OurLoss"; }
  /**
   * Unlike most loss layers, in the EuclideanLossLayer we can backpropagate
   * to both inputs -- override to return true and always allow force_backward.
   */
