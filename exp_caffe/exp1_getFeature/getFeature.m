...
modelName = 'bvlc_reference_caffenet.caffemodel';
deployFileName = 'deploy_701.prototxt';
matcaffe_init(1, deployFileName, modelName);

% read data
imageFolder = '../../data/tempEgoVideoSeq701GT/';
imageFiles = dir([imageFolder '*.jpg']);
imageFiles = {imageFiles.name};

features = zeros(6,6,256,length(imageFiles));

for i = 1:length(imageFiles)
    % prepare imageData
    % input_data is Height x Width x Channel x Num
    im = imread([imageFolder imageFiles{i}]);
...
save('tempEgoVideoSeq701GT', 'features');

% ------------------------------------------------------------------------
function data = prepare_image(im)
% ------------------------------------------------------------------------
...
im = im(:,:,[3 2 1]) - imresize(IMAGE_MEAN, [CROPPED_DIM CROPPED_DIM]);
im = permute(im, [2 1 3]);
